...
_completion_zsh() {
    ...
    # Intentar función de completion específica
    ...
    # Fallback: compadd para archivos/directorios
    compadd -a words - "$word" 2>/dev/null
    compgen -d "$word" 2>/dev/null
    compgen -f -- "$word" 2>/dev/null
}
...
_completion_powershell() {
    ...
    pwsh -NoProfile -Command "
        \$params = @{WordToComplete='$word'; CommandName='$cmd'; CommandType='Cmdlet,Function,Filter,Script'}
        Get-CompletionAtCursor @params 2>\$null | Select-Object -ExpandProperty CompletionText -ErrorAction SilentlyContinue
    " 2>/dev/null
}
